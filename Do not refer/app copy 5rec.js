...
$(() => {
  ...
  const numb = event.currentTarget;
  if(counter.indexOf(numb.id) === -1 || counter.indexOf(event.id) === -1) {
  ...
  console.log(counter);
    ...
    let squareId = numb.id;
    let squarePos = $numb.position();
    console.log(arry);
    $(numb).css('background-color', 'pink')


    // const isLeftEdge = squareId % 10 === 0
    // const isRightEdge = squareId % 10 === 9
    
    // if (squareId>0 && !isLeftEdge) {
    //   console.log('s');
    // }

    // if (squareId<100 && !isRightEdge) {
    //   console.log('y');
    // }
    checkSquare(squareId, squarePos);

    function checkSquare(squareId, squarePos) {
      const isLeftEdge = squareId % 10 === 0
      const isRightEdge = squareId % 10 === 9
    
    if (squareId>0 && !isLeftEdge) {
    const $newId = parseInt(squareId)-1
    console.log($newId);
